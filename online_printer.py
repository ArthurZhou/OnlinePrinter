...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        f.write(b"<hr><small>Powered By: D2Lib</small></body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        ...
        if f:
            ...
            f.close()
        if r:
            os.system('java -jar ../printerHandler.jar ' + fileName)
    ...
    def deal_post_data(self):
        ...
        if not fn[0]:
            return False, "Cannot upload an air!", None
        ...
        while remainbytes > 0:
            ...
            if boundary in line:
                ...
                out.write(preline)
                preline = line
        return False, "Unexpected Ends of data.", None
    ...
    def list_directory(self, path):
        ...
        f = BytesIO()
        f.write(b'<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">')
        f.write("<html>\n<title>Online Printer</title>\n".encode())
        f.write("<body>\n<h2>Online Printer</h2>\n".encode())
        f.write(b"<hr>\n")
        ...
        for name in List:
            ...
            f.write(('<li><a href="%s">%s</a>\n'
                     % (urllib.parse.urlparse(urllib.parse.quote(linkname)).path, html.escape(displayname))).encode())
        f.write(b"</ul>\n<hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
...
try:
    fileServe = http.server.HTTPServer((IP, PORT), SimpleHTTPRequestHandler)
    print('Server started on {0}:{1}.'.format(IP, PORT))
    fileServe.serve_forever()
